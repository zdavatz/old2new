...
- **TensorDock**: SSH VMs via API (`dashboard.tensordock.com/api/v2`). Auth: `Authorization: Bearer $TENSORDOCK_API_KEY`. RTX 4090 at ~$0.33-0.40/hr, RTX 5090 at ~$0.45-0.54/hr. Ubuntu 24.04 base image (no PyTorch pre-installed — cloud-init installs everything). Default SSH user is `user` (not root). API key stored in `~/.bashrc` as `TENSORDOCK_API_KEY`. `tensordock_batch.sh` auto-calculates disk size per video and passes it to instance creation. Requests 16 vCPUs / 32GB RAM for fast parallel frame extraction. Port forwarding maps internal 22→random and 8080→random external ports. Disk resize requires stop→modify→start (GPU may detach — better to create with correct size from the start). RTX 5090 support: override with `GPU_MODEL=geforcertx5090-pcie-32gb`, auto-detects Blackwell arch and installs PyTorch with CUDA 12.8. Organization: old2new.
