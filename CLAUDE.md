...
- **li_push_rs/** — LinkedIn video upload via Videos API + Posts API. OAuth2 auth flow (`--auth`) with OpenID Connect for person ID extraction from JWT id_token. Local callback on port 8092. 4-step upload: initialize → chunked upload (4MB parts with ETags) → finalize → create post. Supports YouTube URL or video ID as input — auto-downloads via yt-dlp with title/description. Pre-checks duration (max 30 min) and filesize (max 500MB). `--low-quality` flag for 1080p fallback when video exceeds 500MB. Shows post URL after publish. Usage: `li_push --auth`, `li_push video.mp4 --title "Title"`, `li_push YOUTUBE_ID`, `li_push "https://youtube.com/..." --low-quality`. Credentials in `linkedin_credentials.json`, token in `linkedin_token.json`. Writes PID to `~/li_push.pid`.
