...
- **deploy.sh** — Deploys videos to cloud GPU instances. Modes: `<ids>` (auto), `new <ids>`, `--instance <ID> <ids>`, `--plan <ids>`, `update <instance_id>` (full redeploy + restart), `update-soft <id>` (upload as .new without restart), `swap-binary <id> <name>` (swap ONE binary, restart only that process — safe during ffmpeg/upscaling), `destroy <instance_id>`. Options: `--single`/`--multi`, `--vastai`/`--tensordock`. **Safe update workflow**: `swap-binary` for dashboard/scheduler updates without disturbing running processes. `status_server` and `gpu_scheduler` auto-restart after swap; other binaries (`enhance`, `frame_gap_check`, `rebalance_calc`, `youtube_upload`) are replaced and used on next invocation. CPU verification: vast.ai boost clock != actual (searches `cpu_ghz>=4.0` for 5090, verifies via SSH). Blacklists bad machines to `blacklist_machines.txt`.
