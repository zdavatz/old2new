...
- **enhance_gpu.py** — Cloud GPU script: same pipeline but uses PyTorch/CUDA for upscaling. Checks disk space before starting. Uses `~/jobs/<video-id>/` for work directories.
