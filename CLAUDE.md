...
- **multi_gpu_queue.sh** — Multi-GPU queue processor. Queue from `~/json/*.json`. Atomic pick via flock + mv to `.processing.{gpu}`. Success → `~/json_done/`, failure → back to `~/json/`. PID-file per GPU, OOM-kill recovery (3 retries). Resume priority: most existing `frames_out` first. Workers poll every 30s. **Auto-destroy**: when queue empty + all idle + email confirmed, 10-min grace then stops instance. Usage: `./multi_gpu_queue.sh [NUM_GPUS]`.
...
- **upscale.py** — Minimal Python (~93 lines), ONLY Real-ESRGAN upscaling. Usage: `python3 upscale.py <frames_in> <frames_out> <scale> [--tile N]`. Auto-tiling, deletes input frames in single-GPU mode (keeps last 10). In segment-split mode (`--end > 0`), inputs NOT deleted (shared dir). Atomic writes via `.tmp.png` + `os.rename`. **Important**: tmp files must use `.tmp.png` (not `.png.tmp`) — OpenCV determines format from extension.
- **pimp_brightness.sh** — End-to-end brightness correction: download from YouTube → adjust segment → re-upload. Only re-encodes the affected segment (gamma correction), stream-copies the rest. Usage: `./pimp_brightness.sh <youtube-url> <start> <end> <percent> [--suffix v1] [--title "Custom Title"]`. Percent: `-25` = 25% darker, `+30` = 30% brighter. `--suffix v1` appends to existing title → "Enhanced 4K v1". Uploads via Rust `youtube_upload` binary, sends email to juerg@davaz.com.
...
- **RTX 5090 HD CPU benchmarks** (1920x1200, 2x, tile=512): Xeon Gold 6530 @ 0.9 GHz = ~0.1 fps (GPU idle!), Xeon 8481C @ 2.0 GHz = 0.3-0.4 fps, Ryzen 9 7950X @ 5.9 GHz = 0.5 fps, Threadripper PRO 7975WX @ 8.2 GHz = 0.5 fps. Above ~5 GHz bottleneck shifts to disk I/O. **Min 3 GHz for RTX 5090 HD, ideal 5+ GHz.**
...
- **Auto-tiling**: RTX 4090/5090 safe up to 1.6 MP without tiling. Above 1.6 MP → tile=512. L40S/A6000 (48GB) up to 2.0 MP, 80GB+ up to 4.0 MP. Tiling **faster** for high-res: 1920x1200 tile=512 = 0.56 fps vs no-tile = 0.13 fps (RealESRGAN processes at 4x internally).
...
- **RTX 5090 optimization**: tile=512 optimal, FP16 67% faster than FP32 (already used), tile_pad=0 = 7% but risks artifacts, torch.compile() incompatible. GPU draws only 208W/575W at 1920x1200 — framework/CPU-limited. Per-res fps: SD 640x480 = 3.27 fps (340W), HD 960x720 = 1.47 fps (281W), HD 1920x1200 = 0.44 fps (208W).
