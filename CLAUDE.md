...
- **youtube_upload.py** — Uploads enhanced video to YouTube, copies title/description from original (adds "Enhanced 4K" suffix), sends email notification to juerg@davaz.com via Gmail API with old + new video links. Requires `client_secret.json` and `youtube_token.json` (OAuth2 with youtube.upload + gmail.send scopes). Token auto-refreshes. Used in the queue pipeline: upscale → upload → notify → delete .mkv to free disk. For video IDs starting with dash (e.g. `-x_aIkSrXFw`), use `--video-id=-x_aIkSrXFw` instead of positional arg (argparse treats leading dash as flag).
...
- **Multi-GPU scales linearly for multiple videos**: 4x RTX 5090 = 1.47 fps combined (4.1x speedup) when processing **different videos** on each GPU ($1.35/hr on vast.ai Sichuan). Each GPU runs its own `enhance_gpu.py` process via `CUDA_VISIBLE_DEVICES`. Process videos in batches of 4, wait for batch, start next 4. Dashboard supports multi-GPU: per-GPU logs, combined fps, temp/util/VRAM.
...
- **Multi-GPU disk sizing**: With 4 GPUs parallel, each GPU needs its own disk budget. 500GB / 4 GPUs = 125GB per GPU. At 1920x1200 2x: max ~5min per video. At 960x720 2x: max ~17min. For short HD videos (<5min), 4x RTX 5090 with 500GB is ideal. For longer videos, use single-GPU instances with more disk.
- **Watchdog for long-running jobs**: vast.ai instances may kill processes unexpectedly. Use a watchdog script that checks every 60s if enhance_gpu.py is still running and restarts it if not (resumable — picks up at last completed frame).
