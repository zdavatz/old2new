...
- **pimp_brightness.sh** — End-to-end brightness correction: download from YouTube → adjust segment → re-upload. Only re-encodes the affected segment (gamma correction), stream-copies the rest. Usage: `./pimp_brightness.sh <youtube-url> <start> <end> <percent> [--title "Custom Title"]`. Percent: `-25` = 25% darker, `+30` = 30% brighter. Uploads via Rust `youtube_upload` binary, sends email to juerg@davaz.com.
