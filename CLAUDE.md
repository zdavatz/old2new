...
- **close_enhanced_issues.py** — Auto-closes GitHub issues for videos that have been uploaded as "Enhanced 4K" to YouTube. Queries YouTube API for enhanced videos, matches them to original video IDs, finds open GitHub issues in zdavatz/old2new, and closes them with a comment linking to the new video. Supports dry-run (default) and `--close` mode. Uses fuzzy title matching with normalization (HTML entities, emoji, punctuation).
...
- **Multi-GPU scales linearly**: 4x RTX 5090 = 1.47 fps combined (4.1x speedup over single GPU, $1.50/hr on vast.ai Sichuan). Each GPU runs its own `enhance_gpu.py` process via `CUDA_VISIBLE_DEVICES`. Uses `multiprocessing.set_start_method('spawn')` (CUDA requires spawn, not fork). Videos distributed round-robin across GPUs. Dashboard (`status_server.py`) supports multi-GPU: reads per-GPU logs (`gpu0.log`, `gpu1.log` etc.), aggregates combined fps, shows per-GPU temp/util/VRAM.
