...
- Auto-tiling for VRAM management: RTX 4090 (24GB) safe up to 1.6 MP, RTX 5090 (32GB) up to 1.8 MP, L40S/A6000 (48GB) up to 2.0 MP, 80GB+ up to 4.0 MP. Higher resolutions automatically use tile=512 or smaller. VRAM scales super-linearly because RealESRGAN_x4plus always processes at 4x internally — 1920x1200 input needs ~50GB VRAM for no-tile mode. Key finding: **no-tile is NOT faster** — at 1920x1200 (2.3 MP), tile=512 gives 0.56 fps while no-tile gives only 0.13 fps on both RTX 5090 (32GB) and RTX Pro 6000 (96GB). Tiling is 4x faster because processing 12 small tiles is more efficient than one huge 7680x4800 internal image.
- GPU power limit matters: RTX Pro 6000 Blackwell "Max-Q" (300W) is slower than RTX 5090 (575W) at same clock speed (3090 MHz) because power throttling limits sustained compute. Pre-flight check now warns about low-power GPUs (< 400W for >30GB VRAM). Always prefer full-power GPUs for Real-ESRGAN.
- Two GPU profiles for the davaz.com collection: **SD-4x** (RTX 4090, 24GB, ≤1.6 MP, 2.6 fps, $0.41/hr) and **HD-2x** (RTX 5090, 32GB, >1.6 MP, ~1.7 fps, $0.75/hr). The script auto-detects and selects the right GPU. A100 is NOT suitable for Real-ESRGAN (0.07 fps due to low clock speeds). As of 2026-03-18: 11 of 226 videos enhanced, 215 remaining (72 SD→RTX 4090 4x, 143 HD→RTX 5090 2x). GPU assignment is by YouTube definition (hd/sd), not by megapixels — all HD videos go to RTX 5090 regardless of resolution.
...
- API keys stored in `~/.bashrc` as `TENSORDOCK_API_KEY` and `RUNPOD_API_KEY`, in `~/.zshrc` as `VAST_API_KEY`
