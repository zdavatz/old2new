...
- **Fade-out** (1.0.26+): a checkbox (default **on**) that freezes the last frame for 10 seconds and fades it — picture and sound — to black/silence at the end of every short. The short ends up 10 s longer. The faded variant is cached next to its source segment so Preview → Upload doesn't re-encode. Applies to both Preview and Upload.
