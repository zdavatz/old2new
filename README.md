...
- **Title overlay** (1.0.24+): a checkbox + color picker burns the title into the bottom-left corner of every frame for 3 seconds (starting 1 s in). Rendered in pure Rust and composited with ffmpeg's `overlay` filter so it works on Homebrew's stock ffmpeg (no `drawtext`/libfreetype needed). Applies to both Preview and Upload.
