...
- **Direct upload tab** (1.0.19+): an **⬆ Upload** button next to 📜 History opens a modal for uploading a local video file straight to YouTube — no yt-dlp, no segment extraction. Fields: file path (drag-and-drop a video into the window to set it), title, description, privacy. Reuses the same OAuth + resumable upload code path as the create-shorts flow, streams progress into the modal's own log + progress bar, and records the result in the same `uploads.jsonl` history file.
