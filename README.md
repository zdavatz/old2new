...
- **Fade-in** (1.0.28+): a checkbox (default **off**) with its own duration field (1–60 s, default 10) that freezes the first frame and fades it — picture and sound — in from black/silence at the start, then the movie plays from that frame. Symmetric to Fade-out; the two can be combined (fade-in opens, fade-out closes). Applies to both Preview and Upload.
