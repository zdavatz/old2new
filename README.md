...
- **Fade-out** (1.0.26+): a checkbox (default **on**) that freezes the last frame and fades it — picture and sound — to black/silence at the end of every short. Since **1.0.27** an adjacent field sets the duration in seconds (1–60, default 10), so it's no longer fixed at 10 s; the short ends up that many seconds longer. The faded variant is cached next to its source segment (keyed on the duration) so Preview → Upload doesn't re-encode. Applies to both Preview and Upload.
