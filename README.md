...
- **Fade-out** (1.0.26+): a checkbox (default **on**) that fades the end of every short to black, with the music playing the whole time and getting quieter as the picture dims. Since **1.0.27** an adjacent field sets the duration in seconds (1–60, default 10). The faded variant is cached next to its source segment (keyed on the duration) so Preview → Upload doesn't re-encode. Applies to both Preview and Upload. *(Up to 1.0.30 this froze the last frame and the music stopped during the fade; **1.0.31** plays the real music through the fade and no longer lengthens the clip.)*
