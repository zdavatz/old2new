...
- **Send via WhatsApp** (1.0.11+): after a successful upload, the success banner shows a **Send via WA** button that pushes the YouTube link (with title) to a configured WhatsApp recipient. Self-contained — Settings has a **Setup WhatsApp** button that materialises bundled `send-text.mjs` / `login.mjs` + `package.json` into `<config_dir>/whatsapp/` and runs `npm install` (Baileys + qrcode-terminal + pino), and a **Link WhatsApp** button that runs the QR-code flow inside a modal so you can link the device with WhatsApp on your phone (Settings → Linked Devices → Link a Device). 1.0.14 ports the link flow from `~/software/pegelstand/whatsapp/login.mjs` (success on first `connection: open`, reconnect once on status 515, wipe + retry on 401/403) after earlier in-house variants in 1.0.11–1.0.13 had the phone reject the device with *"Gerät konnte nicht hinzugefügt werden"* even when terminal said it linked. No pegelstand dependency, no shared client. Recipient is a phone number (digits, e.g. `41791234567`) or a group JID (`...@g.us`). Requires Node.js + npm in `PATH`.
